...
def kMeans(K, is_valid=False):
  ...
  #   print(assign_indices.shape)
  loss_mu = tf.reduce_sum(mins, 0)
  adam_op = tf.train.AdamOptimizer(learning_rate=0.1, beta1=0.9, beta2=0.99, epsilon=1e-5).minimize(loss_mu)
  ...
  valid_loss = []
  for i in range(500):
    _, MU_value, assignment_values, loss, percentages_value = sess.run(
      [adam_op, MU, assign_indices, loss_mu, percentages], feed_dict={X: train_data})
...
#   plt.subplot(122)
#   plt.scatter(train_data[:, 0], train_data[:, 1], c=assignment_values, s=1, alpha=0.8)
#   plt.plot(MU_value[:, 0], MU_value[:, 1], 'r^', markersize=8)
#   plt.title('K = {}'.format(K))
#   plt.savefig('loss_scatter_with_{}.png'.format(K))
#   files.download('loss_scatter_with_{}.png'.format(K))

#   plt.savefig('val_loss_scatter_with_{}.png'.format(K))
#   files.download('val_loss_scatter_with_{}.png'.format(K))
#   plt.savefig('Kmean_compare_scatter_with_{}.png'.format(K))
...
kMeans(4, is_valid=True)
kMeans(5, is_valid=True)
kMeans(6, is_valid=True)
kMeans(10, is_valid=True)
kMeans(15, is_valid=True)
kMeans(20, is_valid=True)
kMeans(30, is_valid=True)
plt.show()

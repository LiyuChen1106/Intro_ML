...
def GMM(K, is_valid=False):
    ...
    loss = - tf.reduce_sum(hlp.reduce_logsumexp(log_gauss + tf.transpose(log_pi), 1), axis=0)
    ...
    for i in range(500):
        _, MU_value, log_pi_values, variance, loss_value, log_post_value, predictions, percentages_value = sess.run(
            [adam_op, MU, log_pi, sigma_sq, loss, log_post, assign_predict, percentages], feed_dict={X: train_data})
...
#   plt.savefig('gmm_loss_with_{}.png'.format(K))
...
#   plt.savefig('gmm_loss_scatter_with_{}.png'.format(K))
#   files.download('gmm_loss_scatter_with_{}.png'.format(K))

#   plt.savefig('gmm_val_loss_scatter_with_{}.png'.format(K))
#   files.download('gmm_val_loss_scatter_with_{}.png'.format(K))
...
# # GMM(8, is_valid=True)
GMM(9, is_valid=True)
GMM(10, is_valid=True)
GMM(11, is_valid=True)
GMM(15, is_valid=True)
GMM(20, is_valid=True)
GMM(30, is_valid=True)
